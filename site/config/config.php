...
return [
  ...
  // SEO routes.
  // `sitemap.xml` is generated by bnomei/kirby3-feed from the page index.
  // The closure passed to sitemap() defers building the collection until
  // after a cache miss, so cache hits stay cheap (per the plugin's docs).
  // `index()` returns all published pages (drafts excluded). We don't use
  // `listed()` here because this site's pages carry no sort-number prefixes
  // — they're all "unlisted" but real, public pages. The `media` library
  // page is excluded (it's an asset container, not real content).
  // `robots.txt` allows everything and points crawlers at the sitemap
  // (Kirby serves no robots.txt by default).
  'routes' => [
    [
      'pattern' => 'sitemap.xml',
      'method'  => 'GET',
